...
Left avatar, right body. Weibo: name + optional V, body with `--sa-entity` for @/# (or `#topic#` on Weibo), nine-grid, then time / 来自 / 查看原文. Live photos occupy one cell: still JPEG, hover plays the motion, `LIVE` badge bottom-left (10px/700, 18px tall, `rgba(0,0,0,0.55)`). Regular video cells are 16:9 with native controls. X: name + @handle + · time + 打开原文, body with `--sa-entity` entities, rounded media; GIFs and videos use controls, never a LIVE badge. Instagram: desktop-style split card — black media panel on the left with prev/next arrows and dot indicators for carousels; right sidebar with avatar + username + @handle, scrollable caption (`username` bold + text), then muted time and 查看原文; no LIVE badge; in-app cards and offline HTML share this layout. If the window cannot decode a file, the cell becomes a short fallback with `color: inherit` — accent pill 「用系统播放器打开」 / 「打开实况」. No live 转发/评论/赞 or reply/repost/like row — those are not actionable in a local archive.
