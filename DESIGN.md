...
### App icon
Rounded-square desktop mark on charcoal (`#111111`). Center motif: four fanned layers in Weibo Saffron, Sky Link cyan, Instagram magenta, and warm pink — stacked cards suggesting archived feeds, not a literal platform logo. Source PNGs live in `icons/social-archiver-icon-1024.png` (and `-512`); `scripts/write_icon.py` emits `icons/icon.ico` for Electron. No text in the glyph; readable at 16px.
